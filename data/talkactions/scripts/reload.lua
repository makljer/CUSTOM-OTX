...
function onSay(player, words, param)
	...
	if not reloadType then
		player:sendTextMessage(MESSAGE_STATUS_CONSOLE_RED, "Reload type not found.")
		return false
	end

	Game.reload(reloadType.targetType)
	player:sendTextMessage(MESSAGE_STATUS_CONSOLE_RED, string.format("Reloaded %s.", reloadType.name))
	return false
end
